...
import { Building2, ClipboardList, LogOut, ChevronRight, Menu, X } from 'lucide-react';
import { useState } from 'react';
import { NavLink, Outlet, useNavigate, useLocation } from 'react-router-dom';
import { useAuthStore } from '../../store/authStore';

// Shared portal shell for Teachers and Students (read-only room access + my reports).
export function ViewerLayout({ role }: { role: 'Teacher' | 'Student' }) {
  ...
  const nav = [
    { to: base,                 icon: Building2,     label: role === 'Teacher' ? 'My Faculties' : 'Rooms', end: true },
    { to: `${base}/my-reports`, icon: ClipboardList, label: 'My Reports' },
  ];
      ...
      <div className="px-6 py-5 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">{role} Portal</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-3 py-4 border-t border-gray-100">
        <div className="flex items-center gap-3 px-3 py-2 mb-1">
          <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-semibold text-sm">
            {user?.email?.[0]?.toUpperCase() ?? role[0]}
          </div>
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="lg:hidden flex items-center gap-4 px-4 py-3 bg-white border-b border-gray-200">
          ...
          <span className="font-semibold text-gray-900">UAMS {role}</span>
        </header>
